...
class BaseModel(torch.nn.Module):
    ...
    def _remap_cls_by_names(self, csd, src_model, verbose=True):
        ...
        # Skip default placeholder names {0:"0", 1:"1", ...} (also catches empty dicts) 鈥?nothing to match on
        ...
        valid = idx >= 0
        state_dict = self.state_dict()
        # Exact class-logit conv weight/bias keys from the detection head(s) 鈥?restricting to these avoids
...
class _SafeLoad:
    ...
    def activation(act):
        ...
        try:
            ...
            raise TypeError(
                emojis(f"ERROR 鉂岋笍 unsupported activation '{act}' blocked during restricted model load.")
            ) from e
    ...
    def _build(cls):
        ...
        Returns:
            (list): Items for `torch.serialization.safe_globals` 鈥?classes and `(obj, "module.Name")` aliases.
        """
        import enum
        import importlib
        import inspect
        ...
        def _scan(pkg):
            ...
            if hasattr(pkg, "__path__"):  # package: include all submodules
                for info in pkgutil.iter_modules(pkg.__path__, f"{pkg.__name__}."):
                    ...
                    except Exception:  # optional/oddball submodule 鈥?skip
                        continue
            for mod in mods:
                for name, klass in inspect.getmembers(mod, inspect.isclass):
                    if issubclass(klass, nn.Module):
                        # Register under the path the class is reachable from 鈥?matches how a checkpoint pickled it.
...
def torch_safe_load(weight, safe_only=None):
    ...
    try:
        ...
        if e.name in {"models", "models.yolo", "models.common", "models.experimental"}:
            ...
            raise ModuleNotFoundError(
                emojis(
                    f"ERROR 鉂岋笍 {weight} requires missing Ultralytics module '{e.name}'. "
        ...
        raise TypeError(
            emojis(
                f"ERROR 鉂岋笍 {weight} references types outside the supported Ultralytics checkpoint format. "
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if restricted and not (isinstance(m, type) and issubclass(m, torch.nn.Module)):
            ...
            raise TypeError(emojis(f"ERROR 鉂岋笍 module '{m}' is not a permitted model layer under restricted loading."))
